...
class WPGH_Core
{
    ...
    static function replaceCallback($matches)
    {
        ...
        foreach($projects as $project)
        {
            ...
            $template = str_replace('{{PROJECT_NAME}}', htmlspecialchars($project->name), $template);
            $template = str_replace('{{PROJECT_WATCHER_COUNT}}', $project->watchers, $template);
            $template = str_replace('{{PROJECT_DESCRIPTION}}', htmlspecialchars($project->description), $template);
